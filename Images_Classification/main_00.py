...
def main():
    ...
    path = "/home/max/WORK/Images_Classification/"
    path_folder = os.path.join(path, 'models')
    model_path = os.path.join(path_folder, 'EXTRACLEARED_ttv_CUT_NEW_max_50_R18_ADAM_StepLR.pt')
    data_dir = os.path.join(path, "EXTRACLEARED_ttv_CUT")
    ...
    def imshow(inp, title=None):
        ...
        mean = np.array([0.485, 0.456, 0.406]) # [0.5, 0.5, 0.5]
        std =  np.array([0.229, 0.224, 0.225]) # [0.5, 0.5, 0.5]
        inp = std * inp + mean
        inp = np.clip(inp, 0, 1)
        plt.imshow(inp)
    ...
    def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
        ...
        for epoch in range(num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                if phase == 'train':
                    # scheduler.step()  # StepRL
                    scheduler.step(epoch_loss)  # RoP
                epoch_acc = running_corrects.double() / dataset_sizes[phase]
                list_of_losses[phase].append(epoch_loss)
                list_of_accuracies[phase].append(float(epoch_acc))
                # print(f"Epoch Loss = {epoch_loss}, type = {type(epoch_loss)}")
    ...
    # optimizer_ft = optim.SGD(model_ft.parameters(), lr=0.001, momentum=0.9, nesterov=False) # Best val Acc: 0.546113
    optimizer_ft = optim.Adam(model_ft.parameters(), lr=0.1, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)

    # exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, step_size=7, gamma=0.2, last_epoch=-1) # Best val Acc: 0.546113
    exp_lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_ft, mode='max', factor=0.4, threshold=0.1,
